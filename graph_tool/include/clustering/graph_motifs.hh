...
struct get_all_motifs
{
                    ...
                    Sampler sampler) const
    {
        ...
        #pragma omp parallel for schedule(runtime) if (num_vertices(g) > get_openmp_min_thresh()) \
            private(sig)
        for (size_t i = 0; i < N; ++i)
        {
            ...
            for (size_t j = 0; j < subgraphs.size(); ++j)
            {
                ...
                auto iso_map =
                    ...
                    (num_vertices(usub), size_t(),
                     boost::get(boost::vertex_index, usub));
                ...
                #pragma omp critical (gather)
                {
                    ...
                    if (!skip)
                    {
                        ...
                        if (!found && fill_list)
                        {
                            ...
                            if (collect_vmaps)
                            {
                                motif = &subgraph_list.back();
                                for (auto v : vertices_range(*motif))
                                    iso_map[v] = v;
                            }
                            found = true;
                        }
                        ...
                        if (found && collect_vmaps)
                        {
                            ...
                            auto& vmap = vmaps[pos].back();
                            for (auto v : vertices_range(*motif))
                                vmap[v] = subgraphs[j][iso_map[v]];
                        }
                    }
                }
            }
